SELECT alum.Codigo, 
	...
	madre.Telefono2, 
	madre.Celular, 
	CONCAT(acudiente.Nombre1," ",acudiente.Nombre2) AS ACUDIENTE, 
	acudiente.Tipo_Documento, 
	acudiente.No_Documento, 
	acudiente.email, 
	acudiente.Telefono, 
	acudiente.Telefono2, 
	acudiente.Celular
FROM Grado INNER JOIN Curso ON Grado.Id_Grado = Curso.Grado_Id_Grado
	 ...
	 INNER JOIN persona acudiente ON alum.Grupo_Familiar_ID_Grupo_Familiar = acudiente.Grupo_Familiar_ID_Grupo_Familiar
	 INNER JOIN davivienda ON alum.Codigo = davivienda.codigo 
	 INNER JOIN importacion_cartera on importacion_cartera.codigo = alum.Codigo
WHERE (padre.Tipo_Persona_Id_Tipo_Persona = 2 OR padre.Tipo_Persona_Id_Tipo_Persona = 4 OR padre.Tipo_Persona_Id_Tipo_Persona = 11 OR padre.Tipo_Persona_Id_Tipo_Persona = 10)
	  AND (madre.Tipo_Persona_Id_Tipo_Persona = 12 OR madre.Tipo_Persona_Id_Tipo_Persona = 4 OR madre.Tipo_Persona_Id_Tipo_Persona = 10 OR madre.Tipo_Persona_Id_Tipo_Persona = 11)
	   AND (acudiente.Tipo_Persona_Id_Tipo_Persona = 3 OR acudiente.Tipo_Persona_Id_Tipo_Persona = 4 OR acudiente.Tipo_Persona_Id_Tipo_Persona = 10 OR acudiente.Tipo_Persona_Id_Tipo_Persona = 11)
